...
//        assertEquals("B002", response.getBody().getBranchId());
//    }
    ...
    void read_shouldReturnBranch() {
        ...
        assertEquals("Cape Town Central", response.getBody().getAddress());
    }

    @Test
    void update_shouldUpdateBranch() {
        testBranch.setAddress("Cape Town Waterfront");
        ResponseEntity<Branch> response = branchController.update(testBranch.getBranchId(), testBranch);
        assertNotNull(response.getBody());
        assertEquals("Cape Town Waterfront", response.getBody().getAddress());
    }
