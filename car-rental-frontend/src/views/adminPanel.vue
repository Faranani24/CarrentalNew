...
<template>
  <div class="min-h-screen flex flex-col bg-neutral-100 p-8">
    <div class="max-w-4xl mx-auto w-full">
      <h1 class="text-3xl font-bold text-neutral-800 mb-8 text-center">Admin Panel</h1>
      ...
      <section class="bg-white rounded-xl shadow-lg p-6 mb-8 border border-neutral-200">
        ...
        <form @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="flex flex-col md:col-span-2">
            <label for="description" class="text-sm font-medium text-neutral-600 mb-1">Description</label>
            <textarea id="description" v-model="car.description" rows="4" class="form-input"></textarea>
          </div>
          <div class="flex flex-col md:col-span-2">
            <label for="image-upload" class="text-sm font-medium text-neutral-600 mb-1">Car Image</label>
            <input id="image-upload" type="file" @change="handleImageUpload" required
                   class="form-input file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-600 hover:file:bg-orange-100 cursor-pointer">
          </div>
          ...
          <table class="min-w-full divide-y divide-neutral-200">
            ...
            <tr v-for="carItem in cars" :key="carItem.id">
              <td class="px-4 py-2 whitespace-nowrap">
                <img :src="carItem.imageUrl" :alt="carItem.make" class="w-16 h-12 object-cover rounded-md shadow-sm">
              </td>
              ...
              <td class="px-4 py-2 whitespace-nowrap">
                  <span
                      :class="{
                      'bg-green-100 text-green-800': carItem.status === 'available',
                      'bg-red-100 text-red-800': carItem.status === 'booked',
                      'bg-yellow-100 text-yellow-800': carItem.status === 'maintenance'
                    }"
